...
async def tcp_echo_client(message):
    try:
        ...
        # Keep connection open for random n sec
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-n", type=int, default=10, help="Number of clients (default: %(default)s)"
    )
    parser.add_argument(
        "--host",
        type=str,
        default="127.0.0.1",
        help="Host to bind the server (default: %(default)s)",
    )
    parser.add_argument(
        "--port",
        type=int,
        default=5000,
        help="Port to bind the server (default: %(default)s)",
    )
